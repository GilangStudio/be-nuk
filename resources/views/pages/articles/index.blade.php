    ...
    <form method="GET" action="{{ route('articles.index') }}" id="filter-form">
        <div class="d-flex justify-content-start align-items-center gap-2">
            ...
            <div class="d-flex gap-2 align-items-center">
                ...
                <a href="{{ route('articles.index') }}" class="btn btn-secondary btn-icon" data-bd-title="Clear all filters">
                    <i class="ti ti-x"></i>
                </a>
                @endif
            </div>
...
<div class="col-12">
    <div class="card">
                            ...
                            <td colspan="5" class="text-center py-5">
                                <div class="empty">
                                    <div class="empty-icon">
                                        ...
                                        @else
                                        <i class="ti ti-file-text icon icon-lg"></i>
                                        @endif
                                    </div>
                                    ...
                                    <p class="empty-subtitle text-secondary">
                                        @if(request('search') || request('status') || request('page'))
                                        Try adjusting your search terms or clear the filters to see all articles.
                                        @else
                                        Get started by creating your first article.<br>
                                        Share insights, tutorials, and knowledge with your audience.
                                        @endif
                                    </p>
                                    <div class="empty-action">
                                        ...
                                        @else
                                        <a href="{{ route('articles.create') }}" class="btn btn-primary">
                                            <i class="ti ti-plus me-1"></i> Create First Article
                                        </a>
                                        @endif
                                    </div>
                                </div>
        ...
        @if($articles->total() > 0 || request('search') || request('status'))
        <div class="card-footer d-flex align-items-center">
            <div class="text-secondary">
                @if($articles->total() > 0)
                    Showing <strong>{{ $articles->firstItem() }}</strong> to <strong>{{ $articles->lastItem() }}</strong> 
                    of <strong>{{ $articles->total() }}</strong> results
                    @if(request('search'))
